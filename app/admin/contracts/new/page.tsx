      ...
      <div className="admin-main" style={{ background: "#0A0A0A", padding: "40px 48px", fontFamily: "'Zain', sans-serif", direction: "rtl" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "32px" }}>
          <div>
            ...
            <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.35)", margin: "4px 0 0", fontFamily: "Space Mono, monospace" }}>
              وفق نظام المعاملات المدنية السعودي
            </p>
          </div>
        ...
        <div style={sec}>
          ...
          <div style={{ width: "200px" }}>
            <label style={lbl}>الحالة الأولية</label>
        ...
        <div style={sec}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
            {providerType === "institution" && (
              <>
                <div><label style={lbl}>اسم المؤسسة / الشركة *</label><input value={providerEntityName} onChange={e => setProviderEntityName(e.target.value)} style={inp} placeholder="مؤسسة الأعمال الرقمية" /></div>
                <div><label style={lbl}>اسم المالك / الممثل القانوني</label><input value={providerOwnerName} onChange={e => setProviderOwnerName(e.target.value)} style={inp} /></div>
                <div><label style={lbl}>رقم السجل التجاري</label><input value={providerCR} onChange={e => setProviderCR(e.target.value)} style={{ ...inp, direction: "ltr" }} placeholder="1234567890" /></div>
                <div><label style={lbl}>رقم ضريبة القيمة المضافة (اختياري)</label><input value={providerVAT} onChange={e => setProviderVAT(e.target.value)} style={{ ...inp, direction: "ltr" }} placeholder="300000000000003" /></div>
              </>
            )}
            {providerType === "individual-saudi" && (
              <>
                <div><label style={lbl}>الاسم الكامل الرباعي *</label><input value={providerFullName} onChange={e => setProviderFullName(e.target.value)} style={inp} /></div>
                <div><label style={lbl}>رقم الهوية الوطنية</label><input value={providerNationalId} onChange={e => setProviderNationalId(e.target.value)} style={{ ...inp, direction: "ltr" }} placeholder="1000000000" /></div>
                <div><label style={lbl}>رقم وثيقة العمل الحر</label><input value={providerFreelanceDoc} onChange={e => setProviderFreelanceDoc(e.target.value)} style={{ ...inp, direction: "ltr" }} placeholder="FL-2026-XXXXXX" /></div>
                <div><label style={lbl}>المهنة / النشاط</label><input value={providerProfession} onChange={e => setProviderProfession(e.target.value)} style={inp} placeholder="مصمم جرافيك — تسويق رقمي" /></div>
                <div><label style={lbl}>رقم ضريبة القيمة المضافة (اختياري)</label><input value={providerVAT} onChange={e => setProviderVAT(e.target.value)} style={{ ...inp, direction: "ltr" }} placeholder="300000000000003" /></div>
              </>
            )}
            {providerType === "individual-expat" && (
              <>
                ...
                <div><label style={lbl}>رقم الإقامة</label><input value={providerIqama} onChange={e => setProviderIqama(e.target.value)} style={{ ...inp, direction: "ltr" }} placeholder="2000000000" /></div>
                ...
                <div><label style={lbl}>رقم السجل التجاري (إن وُجد)</label><input value={providerCR} onChange={e => setProviderCR(e.target.value)} style={{ ...inp, direction: "ltr" }} /></div>
                <div><label style={lbl}>رقم ضريبة القيمة المضافة (اختياري)</label><input value={providerVAT} onChange={e => setProviderVAT(e.target.value)} style={{ ...inp, direction: "ltr" }} /></div>
              </>
            )}
            <div><label style={lbl}>رقم الجوال</label><input value={providerPhone} onChange={e => setProviderPhone(e.target.value)} style={inp} placeholder="+966 5X XXX XXXX" /></div>
        ...
        <div style={sec}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
            {clientType === "institution" ? (
              <>
                <div><label style={lbl}>اسم المؤسسة / الشركة *</label><input value={clientCompany} onChange={e => setClientCompany(e.target.value)} style={inp} /></div>
                <div><label style={lbl}>اسم الممثل القانوني *</label><input value={clientName} onChange={e => setClientName(e.target.value)} style={inp} /></div>
              ...
              <>
                ...
                <div><label style={lbl}>رقم الهوية الوطنية / الإقامة</label><input value={clientNationalId || clientIqama} onChange={e => { setClientNationalId(e.target.value); setClientIqama(""); }} style={{ ...inp, direction: "ltr" }} /></div>
        ...
        <div style={sec}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
            ...
            <div><label style={lbl}>Service Description (English)</label><textarea value={serviceDescription} onChange={e => setServiceDescription(e.target.value)} rows={3} style={{ ...inp, resize: "vertical", direction: "ltr", fontFamily: "Space Mono, monospace", fontSize: "11px" }} /></div>
          </div>
        </div>
        ...
        <div style={sec}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "14px" }}>
            ...
            <div>
              ...
              <select value={paymentTerms} onChange={e => setPaymentTerms(e.target.value as typeof paymentTerms)} style={{ ...inp, cursor: "pointer" }}>
                ...
                <option value="50-50">٥٠٪ + ٥٠٪ (دفعتان)</option>
                <option value="3-installments">ثلاث دفعات متساوية</option>
                <option value="custom">مخصص</option>
              </select>
            </div>
            <div><label style={lbl}>عدد جولات المراجعة</label><input type="number" value={revisions} onChange={e => setRevisions(e.target.value)} style={{ ...inp, direction: "ltr", fontFamily: "Space Mono, monospace" }} /></div>
          </div>
          {paymentTerms === "custom" && (
          ...
          <div style={{ marginTop: "14px" }}>
            <label style={lbl}>غرامة التأخير اليومية (ريال) — اختياري</label>
            <input type="number" value={penaltyPerDay} onChange={e => setPenaltyPerDay(e.target.value)} placeholder="0 = بدون غرامة" style={{ ...inp, direction: "ltr", fontFamily: "Space Mono, monospace", maxWidth: "220px" }} />
          </div>
        </div>
        ...
        <div style={sec}>
          ...
          {deliverables.map((d, i) => (
            <div key={i} style={{ display: "flex", gap: "8px", marginBottom: "8px" }}>
              <input value={d} onChange={e => updateDeliverable(i, e.target.value)} style={{ ...inp, flex: 1 }} placeholder={`التسليم ${i + 1}`} />
