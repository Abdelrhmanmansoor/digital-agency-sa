...
import { Invoice, InvoiceItem, PaymentMethodType, PaymentRecord, AdditionalCost, PaymentMethod, PaymentOption } from "@/lib/db";
  ...
  useEffect(() => {
    fetch(`/api/admin/invoices/${id}`)
      ...
      .then((inv: Invoice | null) => {
        ...
        setTitleAr(inv.titleAr ?? "");
        setTitleEn(inv.titleEn ?? "");
        setThankYouAr(inv.thankYouAr ?? "");
        setDiscount(inv.discount ?? 0);
        setDiscountType(inv.discountType ?? "amount");
        setPaymentOptions(inv.paymentOptions ?? []);
        ...
        <div style={sectionStyle}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
            <div>
              ...
              <input value={titleAr} onChange={(e) => setTitleAr(e.target.value)} placeholder="خدمات تصميم وتطوير متجر إلكتروني" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>العنوان (إنجليزي)</label>
              <input value={titleEn} onChange={(e) => setTitleEn(e.target.value)} placeholder="E-commerce store design & development" style={{ ...inputStyle, direction: "ltr" }} />
            </div>
          </div>
          <div style={{ marginTop: "16px" }}>
            <label style={labelStyle}>كلمة الشكر في التذييل (اختياري)</label>
            <input value={thankYouAr} onChange={(e) => setThankYouAr(e.target.value)} placeholder="شكرًا لثقتكم بنا." style={inputStyle} />
          </div>
        </div>
